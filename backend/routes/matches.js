...
router.post('/', authenticateToken, (req, res) => {
  try {
    ...
    const target = queryOne('SELECT id, role FROM users WHERE id = ?', [target_id]);
    ...
    const existing = queryOne('SELECT id FROM matches WHERE parent_id = ? AND grandparent_id = ? AND status != ?', [parent_id, grandparent_id, 'declined']);
    if (existing) return res.status(409).json({ error: 'Es besteht bereits eine Anfrage.' });

    const id = uuidv4();
    runSql(`INSERT INTO matches (id, parent_id, grandparent_id, status, message) VALUES (?, ?, ?, 'pending', ?)`, [id, parent_id, grandparent_id, message || null]);
...
router.get('/', authenticateToken, (req, res) => {
  try {
    const matches = queryAll(`
      ...
      WHERE m.parent_id = ? OR m.grandparent_id = ? ORDER BY m.updated_at DESC
...
router.put('/:id', authenticateToken, (req, res) => {
  try {
    ...
    runSql('UPDATE matches SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [status, req.params.id]);
